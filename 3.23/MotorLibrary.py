...
def setTilt(current, desired, ap2, an2, bp2, bn2):
    if desired == 'neutral':
        if current == 'up':
            halfTiltDown(ap2, an2, bp2, bn2)
        elif current == 'down':
            halfTiltUp(ap2, an2, bp2, bn2)
        return desired
    elif desired == 'up':
        if current == 'neutral':
            halfTiltUp(ap2, an2, bp2, bn2)
...
def sweep(ap1, an1, bp1, bn1, ap2, an2, bp2, bn2, glbl):
    ...
    while glbl.targetFound == False and sweepCount < sweepNum:
        if direction == 'ccw' and sequences > -1*seqLimit:
            oneQuickLeftTurn(ap1, an1, bp1, bn1, delayS)
            sequences = sequences - 1
            
        elif sequences == -1*seqLimit:
            direction = 'cw'
            sequences = sequences + 1
            if tilted == 'up':
        ...
        elif direction == 'cw' and sequences < seqLimit:
            oneQuickRightTurn(ap1, an1, bp1, bn1, delayS)
            sequences = sequences + 1
            
        elif sequences == seqLimit:
            sequences = sequences - 1
            direction = 'ccw'
            snakes = snakes + 1
            if snakes == 2:
    ...
    if glbl.targetFound == True:
        ...
        time.sleep(delayS)
        while abs(glbl.centerFrame[0]-glbl.center[0])>40:
            if (glbl.centerFrame[0] - glbl.center[0]) < 40 and sequences < seqLimit:
                oneQuickRightTurn(ap1, an1, bp1, bn1, delayS)
                sequences = sequences + 1
            elif (glbl.centerFrame[0] - glbl.center[0]) > 40 and sequences > -1*seqLimit:
                oneQuickLeftTurn(ap1, an1, bp1, bn1, delayS)
